...
public class DocsUploadFinalCallback implements DocsUploadFinalInterface {
    ...
    @Override
    public void capturePropertyOrLandPhoto(DocsUploadFinalViewModel viewModel) {
    ...
    @Override
    public void captureServingDPRPhoto(DocsUploadFinalViewModel viewModel) {
        if (checkPermission_API30()){
            requestPermissions(activity, new String[]{Manifest.permission.CAMERA}, IMAGE_CAPTURE_REQ);
        } else {
    ...
    public void takePicturePropertyOrLand(DocsUploadFinalViewModel viewModel) {
    ...
    private File createMediaFilePropertyOrLand(DocsUploadFinalViewModel viewModel) throws IOException {
    ...
    public void takePictureServingNotice(DocsUploadFinalViewModel viewModel) {
        Intent takePictureIntent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
        if (takePictureIntent.resolveActivity(activity.getPackageManager()) != null) {
            File photoFile = null;
            try {
    ...
    private File createMediaFileServingNotice(DocsUploadFinalViewModel viewModel) throws IOException {
        String timeStamp = new SimpleDateFormat("yyyyMMdd_HHmmss", Locale.ENGLISH).format(new Date());
        String fileName = "JPEG_" + timeStamp + "_";
...
//        viewModel.mCurrentServingDPRPhotoPath.set(file.getAbsolutePath());
    ...
    @Override
    public void imageProcessPropertyOrLand(DocsUploadFinalViewModel viewModel) {
        try {
            Uri sourceUri = Uri.fromFile(new File(viewModel.mCurrentPropertyOrLandPhotoPath.get()));
            Log.d("sourceUri", ""+sourceUri);
            String timeStamp = new SimpleDateFormat("yyyyMMdd_HHmmss", Locale.ENGLISH).format(new Date());
            String fileName = "JPEG_" + timeStamp + "_";
            String folder_main = "PropertyOrLandPicturesCrop";
...
//            imagetask.execute(viewModel.mCurrentPropertyOrLandPhotoPath.get(), myDir + "/" + Constant.APP_IMAGE);
    ...
    @Override
    public Uri handleUCropResult(Intent data, DocsUploadFinalViewModel viewModel){
        ...
        viewModel.isImageVisible.set(true);
        try {
            String folder_main_PDF = "ServingNoticePDF";
            ...
            com.itextpdf.text.Image image = com.itextpdf.text.Image.getInstance(resultUri.getPath());  // Change image's name and extension.
    ...
    @Override
    public void imageProcessServingDPR(DocsUploadFinalViewModel viewModel) {
        try {
    ...
    class ImageCompressionAsyncTaskPropertyOrLand extends AsyncTask<String, Void, String> {
        ...
        public ImageCompressionAsyncTaskPropertyOrLand(Context context, DocsUploadFinalViewModel viewModel) {
            mContext = context;
    ...
    private void genrateByteDataUsingFilePropertyOrLand(File imageFile, DocsUploadFinalViewModel viewModel) {
    ...
    class ImageCompressionAsyncTaskServingNotice extends AsyncTask<String, Void, String> {
        ...
        public ImageCompressionAsyncTaskServingNotice(Context context, DocsUploadFinalViewModel viewModel) {
            mContext = context;
            this.viewModel = viewModel;
    ...
    private void genrateByteDataUsingFileServingNotice(File imageFile, DocsUploadFinalViewModel viewModel) {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        Bitmap bitmapObject = BitmapFactory.decodeFile(String.valueOf(imageFile));
        bitmapObject.compress(Bitmap.CompressFormat.JPEG, 100, baos);
        byte[] imageBytes = baos.toByteArray();
    ...
    @Override
    public void showImagePropertyOrLand(DocsUploadFinalViewModel viewModel){
    ...
    @Override
    public void showImageServingDPR(DocsUploadFinalViewModel viewModel){
        Dialog settingsDialog = new Dialog(activity);
        settingsDialog.getWindow().requestFeature(Window.FEATURE_NO_TITLE);
        settingsDialog.setContentView(R.layout.image_popup);
        ImageView imageView = settingsDialog.findViewById(R.id.showImg);
